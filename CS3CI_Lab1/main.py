...
with open('citys.csv', newline='') as csvfile:
...
#Defunct Function trying to generate all possible 2OptSwaps for given lists
def twoOptSwapOld(route):
    swappedRoute = []
    tempRoute = []
    #for a in range(1, len(tempRoute) - 2):
       # print("Hello a")
       # for b in range(a + 1, len(tempRoute)):
        #    print("Hello b")
        #    if (a-b) == 1: continue
    for i in range(len(route)-1):
        tempRoute.append(route[i])
    for i in range(len(tempRoute)-2):
        swapRoute = []
        swapPointOne = len(tempRoute)-((i*2)+1)
        swapPointTwo = swapPointOne - 1
        #print("SwapPoint: " + str(swapPointOne))
        for l in range(i+1):    #Appends everything up until the swap
            swapRoute.append(tempRoute[l])
            #print("l: " + str(l))
        #print("SwapRoute before flipped nodes are added: " + str(swapRoute))
        while len(swapRoute) < (len(tempRoute)-1):
            if swapPointOne <= l:
                #print("Alternate Path")
                swapRoute.append(tempRoute[len(tempRoute)-1])
            else:
                #print("Index being appended to swapRoute: " + str(len(tempRoute)-(len(swapRoute)+1)))
                swapRoute.append(tempRoute[len(tempRoute)-(len(swapRoute)+1)])
        #print("SwapRoute before missing nodes added: " + str(swapRoute))
        #swapRoute.append(tempRoute[(swapPoint)])
        for k in range(len(tempRoute)):    #Seems to function correctly
            missing = False
            for c in range(len(swapRoute)):
                if tempRoute[k] == swapRoute[c]:
                    missing = True
            if missing == False:
                swapRoute.append(tempRoute[k])
        #print(swapRoute)
        lastNumber = -1
        flag = True
        i = 0
        #for i in range(len(swapRoute)):
        if len(swapRoute) >= len(route):
            while flag:
                tempNumber = swapRoute[i]
                ...
                if (i >= len(swapRoute)):
                    flag = False
        #print(swapRoute)
        swapRoute.append(route[0])
        swappedRoute.append(swapRoute)
    #print(swappedRoute)
    return swappedRoute
...
class evolutionAlgorithm:
    ...
    def randomParentSelection(self):    #Old and unused
        usedParents = []
        unusedParents = []
        for i in range(self.size):
            unusedParents.append(i)
        print(unusedParents)
        for i in range(int(2)):    #To be replaced with int(self.size/2) instead of constant
            couple = []
            parent1 = random.randint(0, len(unusedParents)-1)
            couple.append(unusedParents.pop(parent1))
            parent2 = random.randint(0, len(unusedParents)-1)
            couple.append(unusedParents.pop(parent2))
            usedParents.append(couple)
        print(usedParents)
    ...
    def evolutionLoop(self, timeToTry):
        ...
        while startClockTime + timeToTry > time.monotonic():  # While there is still time left
            #self.tournamentParentSelection()
...
#-----------------1-----------------
#getCostOfRoute([routeToBeProvidedinSignOff],newCitys)
#-----------------2-----------------
#generateRepeatedRandom(5)
#-----------------3-----------------
#repeatedOptimisation(10, newCitys)
#-----------------4-----------------
#pop = evolutionAlgorithm(10, newCitys)
...
#pop = evolutionAlgorithm(10, newCitys)
#pop.rouletteParentSelection(8)
#pop.evolutionLoop(15)
#twoOptSwap(generateRandomRoute(cityLocations))
#repeatedOptimisation(1, newCitys)